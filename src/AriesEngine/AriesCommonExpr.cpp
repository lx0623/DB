...
BEGIN_ARIES_ENGINE_NAMESPACE
    ...
    AriesDataBufferSPtr ConstExprContentToDataBuffer( const AriesExprContent& content, size_t tupleNum )
    {
        ...
        {
            buffer = aries_acc::CreateDataBufferWithValue( boost::get< aries_acc::Decimal >( content ), tupleNum );
        }
                                                  ...
                                                  bool printConstAsLiteral ) const
    {
        ...
        switch( type )
        {
            ...
            case AriesExprType::DECIMAL:
            {
                ...
                {
                    result = "( *( Decimal* )( constValues[ " + std::to_string( constValues.size() ) + " ] ) )";
                    constValues.push_back( ConstExprContentToDataBuffer( content, 1 ) );
                }
                                                               ...
                                                               bool printConstAsLiteral )
    {
        seq++;
        std::string result;
        std::string content_string = contentToString( ariesParams, constValues, printConstAsLiteral );
        switch( type )
        {
            case AriesExprType::INTEGER:
            case AriesExprType::FLOATING:
